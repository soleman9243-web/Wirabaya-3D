...
# ⚠️ PROTOKOL & ATURAN KERJA ASISTEN AI (STRICT RULES & BEHAVIOR)

> [!IMPORTANT]
> ### 🛑 ATURAN MUTLAK USER: DILARANG MENGAMBIL INISIATIF TANPA PERINTAH EKSPLISIT!
> Asisten AI **TIDAK BOLEH** mengambil keputusan sendiri, memodifikasi kode/material/shader, atau mengubah struktur aset proyek sebelum mendapatkan instruksi atau persetujuan tegas dari User.

## 1. Perilaku AI yang Diwajibkan (Required AI Behavior)
- **Zero Unsolicited Actions (Tanpa Inisiatif Liar)**:
  - Jangan pernah mengubah, menambah, atau menghapus file apa pun di luar yang secara spesifik diperintahkan oleh User.
  - Jika menemukan ide perbaikan, peningkatan performa, atau alternatif solusi, AI **HANYA BOLEH MENYAMPAIKAN OPSI & BERTANYA TERLEBIH DAHULU**, bukan langsung mengeksekusinya.
- **Konfirmasi Bertahap Sebelum Eksekusi**:
  - Jelaskan secara singkat dan padat apa yang akan dilakukan, file mana saja yang akan disentuh, dan tunggu persetujuan tegas (ACC/OK/Lanjutkan) dari User.
- **Isolasi Masalah & Mencegah Efek Samping (Anti Side-Effects)**:
  - Jika User meminta perbaikan pada satu aset (misal: pencahayaan karakter Wirabaya), AI dilarang mengubah shader global atau aset lain yang digunakan bersama (misal: bangunan, lingkungan, rumput) yang berpotensi merusak aset lain (seperti menyebabkan error shader atau objek menjadi ungu/magenta).
- **Verifikasi Kualitas & Bebas Error**:
  - Selalu verifikasi bahwa Unity Editor tidak mengalami error kompilasi shader, syntax error, atau broken references setelah melakukan perubahan yang disetujui.
- WAJIB BERTANYA JIKA DIRASA RAGU / TIDAK PAHAM MAKSUD USER. LEBIH BAIK BERTANYA DARI PADA LANGSUNG EKSEKUSI DAN SALAH.


## 2. Keinginan, Visi & Rencana User (User's Vision & Roadmap)
- **Visual Karakter Wirabaya (Stylized Anime / Ghibli Style)**:
  - Kulit tampak hangat dan cerah natural (tidak kusam dan tidak terbakar).
  - Karakter tidak boleh menjadi siluet hitam pekat (*pitch black*) saat berada di area bayangan atau membelakangi arah cahaya matahari.
  - Warna pakaian (celana biru tua, manset perban, ikat pinggang) dan rambut tetap terlihat jelas lekukan dan detail cel shading-nya.
- **Eksplorasi Gerakan Rambut (Hair Dynamics)**:
  - User ingin rambut karakter dapat bergerak dinamis (baik opsi procedural wind sway via vertex shader yang ringan, maupun opsi rigid spring physics dengan rigging bones di Blender saat User memutuskan siap).
- **Integritas & Keindahan Visual Lingkungan**:
  - Mempertahankan tampilan *stylized grass* (rumput Ghibli dengan upward normal dan fake SSS), serta bangunan/rumah/aset lingkungan lainnya agar tetap stabil, indah, dan bebas dari error shader (warna ungu/magenta).
...
135. Optimasi Pergerakan Karakter di Lereng Gunung (Slope Movement) & Pencerahan Global Volume

A. Deskripsi Permasalahan
   1. Karakter sering tersendat, lambat, atau "nyangkut" saat berjalan menanjak/menuruni lereng bukit dan gunung karena CharacterController default StarterAssets menggerakkan karakter secara murni horizontal (XZ plane) dan menabrak normal miring dari Terrain.
   2. Parameter CharacterController pada prefab memiliki StepOffset kecil (0.25) dan SkinWidth tipis (0.02) yang rentan tersangkut mikro-undakan permukaan Terrain.
   3. Scene terlihat gelap dan kontras rendah akibat belum adanya kompensasi exposure pada URP Global Volume.

B. Rincian Perubahan & Solusi Teknis
   1. ThirdPersonController.cs (Slope Projection & Anti-Stuck):
      - Menambahkan kalkulasi `Vector3.ProjectOnPlane(targetDirection, slopeHit.normal)` ketika karakter berada di tanah (`Grounded`) dan berjalan di lereng (`slopeAngle <= slopeLimit`).
      - Karakter meluncur mulus mengikuti kontur permukaan bukit/gunung tanpa terhambat gaya tabrakan ke dalam lereng.
   2. Parameter Prefab PlayerArmature & PlayerArmature 1:
      - Slope Limit ditingkatkan dari 45° ke 55° (mampu mendaki bukit terjal).
      - Step Offset ditingkatkan dari 0.25 ke 0.45 (mampu melompati undakan dan gundukan batu).
      - Skin Width dioptimalkan ke 0.06 (mencegah tunneling dan jitter kontak dengan Terrain).
   3. Pencerahan Visual Global Volume Profile:
      - Menambahkan komponen `ColorAdjustments` pada `Global Volume Profile_Level0.asset` dengan `postExposure: +0.65 EV` dan `contrast: +12`.
      - Menghasilkan gambar yang jernih, cerah, dan hidup tanpa merubah rotasi posisi asli matahari yang diinginkan user.


136. Sistem Locomotion Karakter AAA ala ALS (Advanced Locomotion System)

A. Deskripsi Permasalahan & Inspirasi
   1. Terinspirasi dari showcase sistem lokomosi realistis tingkat AAA (video referensi Afrizal Aurelio: Advanced Locomotion System in Unity).
   2. Karakter sebelumnya berbelok kaku tanpa ada efek miring tubuh (banking/lean), telapak kaki belum otomatis menempel rata saat berdiri di permukaan miring atau bebatuan, dan saat berlari menuruni lereng bukit curam karakter bisa sedikit melayang/terpental (bouncing) karena sphere check grounded yang lepas sekejap.

B. Rincian Fitur & Solusi Teknis
   1. PlayerFootIK.cs (Penyempurnaan Foot IK & Pelvis Offset):
      - Smart Layer Mask: Di `Awake()`, memastikan Foot IK mendeteksi bebatuan, undakan kayu, dan jembatan (Layer `Default` 0, `Obstacle` 12) di samping bukit (Layer `Ground` 11) sambil mengabaikan Player dan UI.
      - Perlindungan Kemiringan (maxSlopeAngle: 55°): Telapak kaki hanya mengikuti kemiringan lereng yang wajar, mencegah telapak kaki terpuntir 90° saat menempel di dinding/tebing curam.
      - Menggunakan native Mecanim `OnAnimatorIK` dengan interpolasi `SmoothDamp` pada posisi dan rotasi telapak kaki serta panggul (pelvis drop).
   2. PlayerLocomotionLeaning.cs (Procedural Spine & Chest Leaning):
      - Turn Banking (Roll): Saat karakter berlari kencang dan berbelok tajam, tubuh karakter (tulang Spine dan Chest) otomatis miring ke arah belokan (hingga 7.5°) mengikuti inersia gerakan seperti atlet/sepeda motor.
      - Slope Pitch: Saat menanjak lereng bukit curam, tubuh karakter otomatis condong ke depan (hingga 8°) untuk menjaga pusat gravitasi; saat menuruni lereng, tubuh menahan ke belakang.
      - Acceleration & Braking Pitch: Tubuh sedikit condong maju (hingga 3.5°) saat mulai lari cepat dan condong mundur (hingga 3.0°) saat mengerem berhenti.
      - Pembagian Bobot Tulang 50:50: Rotasi didistribusikan merata antara Spine dan Chest sehingga lengkungan tulang belakang tampak lentur dan organik.
      - Safety Fade: Efek leaning otomatis fade-out halus saat melompat di udara (`!Grounded`) atau saat mode combat/roll.
   3. ThirdPersonController.cs (Downhill Sticking & Slope Normal Tracking):
      - Downhill Sticking: Saat karakter berlari kencang menuruni lereng dan sphere check sempat lepas sesaat di udara (< StepOffset), sistem secara otomatis mendeteksi tanah di bawah kaki dan menempelkan karakter kembali ke lereng secara halus sehingga tidak ada bouncing atau stuttering animasi jatuh.
      - Slope Normal Accessor: Menyediakan getter `GetSlopeNormal()` dan `GetSpeed()` untuk diintegrasikan secara presisi dengan sistem leaning dan foot alignment.
   4. Sinkronisasi Prefab & Scene:
      - Komponen `PlayerFootIK` dan `PlayerLocomotionLeaning` telah terpasang rapi dan aktif pada GameObject `PlayerArmature` di scene `#1- 1.unity` serta asset prefab `Assets/Prefabs/PlayerArmature.prefab`.


137. Peningkatan Visual & Shading Karakter Anime (Genshin / Zelda Aesthetic)

A. Deskripsi Permasalahan
   1. Arah pencahayaan rim light pada cel shader karakter sebelumnya terputus dari rotasi matahari karena adanya bug preprocessor `#if MAIN_LIGHT` pada `Cel.hlsl` yang selalu fallback ke vektor statis `(0.5, 0.5, 0)`.
   2. Parameter rim lighting pada material karakter (`kulit.mat`, pakaian, rambut, perban) bernilai negatif `-1` / `-0.01` (mati secara default), sehingga ketika posisi matahari berada di belakang karakter (backlight), siluet tepi karakter tidak memancarkan kilau emas dan terkesan terpotong/gelap.
   3. Threshold pemotongan rim pada `CelShaderV2.shader` terlalu tinggi (`Step(0.5)`), membuat rim light mati di hampir seluruh sudut pandang kamera.
   4. Warna bayangan (`_Shading_Color`) bernilai coklat tanah keruh `(0.42, 0.32, 0.18)` pada kulit dan abu-abu netral mati `(0.49, 0.49, 0.49)` pada pakaian dan rambut, membuat bayangan tampak kotor dan datar.

B. Rincian Perubahan & Solusi Teknis
   1. Perbaikan Bug Shader `Cel.hlsl`:
      - Mengganti blok `#if MAIN_LIGHT` menjadi penetapan langsung `Direction = normalize(light.direction);`. Arah matahari kini 100% akurat mengikuti rotasi directional light di scene.
   2. Optimalisasi Threshold `CelShaderV2.shader`:
      - Menyesuaikan `Unity_Step_float(float(0.2), ...)` di ketiga shader pass agar rim light aktif secara konsisten dan tegas di sepanjang tepian siluet yang membelakangi matahari.
   3. Kalibrasi Material Karakter (`Assets/Asset/Materials/Wirabaya/`):
      - `kulit.mat`:
        * `_Shading_Color`: Diubah ke warna bayangan anime warm peach `(0.72, 0.50, 0.42)` (memberikan efek subsurface scattering yang segar dan hidup).
        * `_Cel_Ramp_Smoothness`: Dinaikkan dari 0 ke `0.08` (menghilangkan garis bayangan kasar berundak pada wajah dan tubuh).
        * `_Rim_Lighting`: Diaktifkan ke `0.35` dengan `_Rim_Brightness: 1.8` (siluet emas memancar indah di bahu dan tubuh).
        * `_Ambient_Self_Lighting`: Diseimbangkan ke `0.20` agar karakter tidak terlalu terang seperti lampu neon namun tetap terbaca jernih saat berada di bawah bayangan pohon.
      - `hair (brown).mat` & `hair (grayish).mat`:
        * `_Shading_Color`: Coklat hangat `(0.38, 0.26, 0.22)` dan slate soft `(0.32, 0.35, 0.42)`.
        * `_Rim_Lighting: 0.35`, `_Rim_Brightness: 2.2` (ujung dan helai rambut berkilau emas saat membelakangi matahari).
      - `cloth (yellowish green).mat` & `cloth (yellowish green but darker).mat`:
        * Dialihkan ke shader cel `CelShaderV2` dan dikalibrasi bayangan amber hangat `(0.46, 0.36, 0.18)` serta rim light `1.8`.
      - `bandage.mat` & `dark blue.mat`:
        * Bayangan lavender lembut `(0.55, 0.52, 0.58)` dan indigo malam `(0.18, 0.22, 0.35)`.
      - `mata.mat`, `pupil.mat`, & `Pupils.mat`:
        * `_Ambient_Self_Lighting` dinaikkan ke `0.85` agar mata karakter selalu berbinar cerah dan ekspresif di segala sudut bayangan.


138. Perbaikan Tuntas Karakter "Nggeret" / Lari di Tempat Saat Menyenggol Dinding & Lantai Kayu

A. Deskripsi Permasalahan
   1. Karakter mengalami kondisi "nggeret" (animasi lari bergerak cepat namun laju fisik karakter macet atau berjalan sangat lambat seperti terjebak lem/lumpur) saat menyenggol dinding rumah kayu, tiang, sudut rintangan, atau lantai berundak.
   2. Penyebab Akar Masalah:
      - Logika bawaan StarterAssets menghitung `_speed` dengan melakukan lerp dari `currentHorizontalSpeed` yang diambil dari `_controller.velocity`. Ketika karakter menyenggol dinding atau sudut lantai, tabrakan fisik menahan `_controller.velocity` mendekati 0. Ini menciptakan feedback loop di mana `_speed` anjlok ke 0.1–0.5 m/s secara permanen, sementara `_animationBlend` tetap naik ke 5.3 m/s (animasi lari penuh tapi fisik jalan di tempat).
      - Proyeksi lereng sebelumnya aktif pada kemiringan sekecil > 0.5°, sehingga celah/seam lantai papan kayu datar terdeteksi miring dan memproyeksikan vektor gerak menancap ke bawah lantai, memicu gesekan tabrakan horizontal.
      - Adanya pemanggilan ganda `_controller.Move` di dalam `GroundedCheck` yang mereset status tabrakan PhysX di tengah frame.

B. Rincian Solusi & Perubahan Teknis ([ThirdPersonController.cs](file:///c:/Users/USER/Documents/GitHub/Wirabaya-3D/Assets/StarterAssets/ThirdPersonController/Scripts/ThirdPersonController.cs))
   1. Akselerasi Kecepatan Mandiri (_speed Independence):
      - `_speed` kini berakselerasi dan melambat secara langsung menuju `targetSpeed` tanpa bergantung pada pembacaan `_controller.velocity`.
      - Saat menyenggol dinding atau berjalan di sudut sempit, karakter tetap mempertahankan daya dorong penuh dan otomatis **meluncur mulus (wall slide)** di sepanjang dinding/rintangan tanpa tersendat atau nggeret.
   2. Pembatasan Proyeksi Lereng Aman:
      - Proyeksi lereng hanya diaktifkan jika berada di lereng bukit nyata (`slopeAngle >= 8.0°`) dan sedang mendaki (`projected.y > 0`). Pada lantai kayu datar, dek rumah, dan tanah datar, arah gerak tetap murni horizontal XZ yang bebas hambatan.
   3. Pembersihan Downhill Sticking & Gravitasi Mantap:
      - Menghapus pemanggilan `_controller.Move` dari `GroundedCheck()` dan meningkatkan tekanan gravitasi tanah saat `Grounded` menjadi `-5.0f`. Karakter menempel rapi di lereng dan undakan tanpa membebani kalkulasi fisika ganda.


139. Perbaikan Keseimbangan Pencahayaan Karakter Anime (Harmonisasi Energi 1/PI & Rona Natural)

A. Deskripsi Permasalahan
   1. Karakter sempat mengalami kondisi terlalu terang/menyilaukan (kulit pucat keputihan dan celana kuning neon menyala seperti lampu bohlam) jika dibandingkan dengan objek di sekitarnya seperti sumur dan gubuk.
   2. Penyebab Akar Masalah:
      - Objek lingkungan standar URP (tanah, sumur, bebatuan) dihitung menggunakan rumus PBR standar yang membagi intensitas matahari dengan PI (`1/PI ≈ 0.318`).
      - Shader cel kustom menyalurkan output pencahayaan langsung ke `surface.Emission` tanpa penskalaan energi tersebut. Ketika Directional Light di-set ke intensitas `5.0`, karakter menerima emisi mentah `5.0`, menjadikannya **3 kali lebih terang** dibanding objek apa pun di sekitarnya.
      - Nilai `_Ambient_Self_Lighting` sebesar `0.48` dan rona warna dasar yang terlalu terang memperparah efek overexposure.

B. Rincian Solusi & Perubahan Teknis
   1. Penskalaan Energi Cel Shading ([Cel.hlsl](file:///c:/Users/USER/Documents/GitHub/Wirabaya-3D/Assets/Asset/NekoLegends/Shaders/ShaderAnimeCel/Cel.hlsl)):
      - Menambahkan penskalaan energi harmonis: `float3 balancedMainLight = light.color * 0.35;`
      - Menggunakan interpolasi bersih: `half3 rampMix = lerp(ToonRampTinting, half3(1, 1, 1), toonRampMain);`
      - Sekarang kecerahan sinar matahari pada karakter menyatu 100% selaras dengan kecerahan sumur, gubuk, dan tanah di sekitarnya.
   2. Kalibrasi Material Kulit ([kulit.mat](file:///c:/Users/USER/Documents/GitHub/Wirabaya-3D/Assets/Asset/Materials/Wirabaya/kulit.mat)):
      - `_Ambient_Self_Lighting`: Diseimbangkan ke level ideal `0.28` (terbaca jernih di bayangan tanpa menyala).
      - `_Color`: Disesuaikan ke rona kulit sawo matang hangat alami `(0.80, 0.67, 0.54)`.
      - `_Shading_Color`: Bayangan umber lembut `(0.58, 0.48, 0.40)`.
      - `_Rim_Lighting`: Dinonaktifkan (`-1`).
   3. Kalibrasi Pakaian & Rambut:
      - `cloth (yellowish green).mat`: `_Ambient_Self_Lighting: 0.25`, `_Color: (0.68, 0.58, 0.28)` (khaki mustard hangat elegan).
      - `cloth (yellowish green but darker).mat`: `_Ambient_Self_Lighting: 0.25`, `_Color: (0.54, 0.44, 0.22)`.
      - `hair (brown).mat` & `hair (grayish).mat`: `_Ambient_Self_Lighting: 0.25`, warna cokelat alami yang teduh.


140. Kalibrasi Kontras Pencahayaan Cel Shading Anime 2.5:1 & Harmonisasi Rona Sawo Matang Alami

A. Deskripsi Permasalahan
   1. Karakter Wirabaya terlihat terlalu cerah (overexposed di level piksel R=253) dan celana tampak kuning neon menyala (`R=204, G=136, B=0`), kontras dengan lingkungan sekitar (sumur R=72-112, tanah R=27-97) yang berada di bawah profil Post-Exposure `-1.2 EV` dan saturasi `+20`.
   2. Penyebab Akar Masalah:
      - Pada iterasi sebelumnya, `ToonRampOutput` menghasilkan nilai `2.03` di area terang dan `1.30` di area bayangan karena direct light tidak dipisahkan sepenuhnya dari bayangan. Akibatnya karakter hampir tidak memiliki kedalaman bayangan (flat seperti stiker kartun menyala).
      - Warna dasar celana (`cloth (yellowish green)`) memiliki rona kuning tinggi yang teramplifikasi oleh saturasi post-processing.

B. Rincian Solusi & Perubahan Teknis
   1. Formula Pencahayaan Cel Shading Seimbang ([Cel.hlsl](file:///c:/Users/USER/Documents/GitHub/Wirabaya-3D/Assets/Asset/NekoLegends/Shaders/ShaderAnimeCel/Cel.hlsl)):
      - Direct Sunlight: `directLit = light.color * 0.21;` (~1.05 intensitas terarah di bawah matahari).
      - Cel Shadow: `shadowLit = (light.color * 0.08) * ToonRampTinting;` (~0.42 intensitas bayangan anime yang lembut, berdimensi, dan tidak siluet hitam).
      - Interpolasi Bersih: `celLighting = lerp(shadowLit, directLit, toonRampMain);`
      - Ambient Lembut: `ambientTerm = Ambient * 0.35;`
      - Menghasilkan rasio kontras anime ideal **2.5 : 1** (area terang ~145-160 sRGB, area bayangan ~75-85 sRGB).
   2. Kalibrasi Warna Kulit Sawo Matang Natural ([kulit.mat](file:///c:/Users/USER/Documents/GitHub/Wirabaya-3D/Assets/Asset/Materials/Wirabaya/kulit.mat)):
      - `_Color`: `{r: 0.73, g: 0.55, b: 0.39, a: 1}` (rona sawo matang khas Nusantara yang hangat, tidak pucat, tidak neon oranye).
      - `_Shading_Color`: `{r: 0.70, g: 0.60, b: 0.52, a: 1}` (bayangan lembut seimbang).
      - `_Ambient_Self_Lighting`: `0.15`.
      - `_Cel_Shader_Offset`: `0.48`, `_Cel_Ramp_Smoothness`: `0.05`.
   3. Kalibrasi Pakaian Celana & Trim ([cloth (yellowish green).mat](file:///c:/Users/USER/Documents/GitHub/Wirabaya-3D/Assets/Asset/Materials/Wirabaya/cloth%20(yellowish%20green).mat)):
      - `_Color`: `{r: 0.50, g: 0.40, b: 0.20, a: 1}` (kain katun/linen khaki-bumi alami, bebas dari warna kuning mencolok).
      - `_Shading_Color`: `{r: 0.65, g: 0.55, b: 0.35, a: 1}`.
      - `_Ambient_Self_Lighting`: `0.15`.
   4. Kalibrasi Rambut, Perban, & Dark Blue:
      - `hair (brown).mat` & `hair (grayish).mat`: Cokelat tua natural `(0.24, 0.18, 0.13)` dan abu teduh `(0.32, 0.32, 0.32)`.
      - `bandage.mat`: Kain perban natural `(0.65, 0.63, 0.60)`.
      - `dark blue.mat`: Biru gelap teduh `(0.14, 0.19, 0.28)`.
...
## Point 141 — Brightness Fix Karakter (Cerahkan dari Hitam)

**Masalah**: Karakter terlalu gelap/hitam setelah kalibrasi 2.5:1 sebelumnya (directLit: 0.21, mbientTerm: Ambient * 0.35).

**Solusi** di Cel.hlsl:
- directLit:  .21 ? ** .28** (lebih terang natural Ghibli)
- shadowLit:  .08 ? ** .10** (bayangan tidak hitam, kontras 2.8:1)
- mbientTerm: Ambient * 0.35 ? **Ambient * 0.6** (indirect light lebih lembut mengisi bayangan)
- extraLights:  .21 ? ** .28**

**Material karakter** — _Ambient_Self_Lighting naik  .15 ? ** .22** untuk semua material (kulit, hair, cloth, bandage, dark blue).

---

## Point 142 — Stylized Grass Aesthetics (Ghibli / Anime)

Berdasarkan riset teknis dari referensi video: Implementasi 3 pilar estetika rumput stylized.

### A. Spherical / Upward Normals
**File**: Assets/InfiniteGrass/Shaders/GrassBladeShader.shader

Normal bilah rumput tidak dibiarkan menghadap ke samping (bayangan hitam berisik). Di-lerp ke loat3(0,1,0) via _NormalUpwardBias (default 0.65):
`hlsl
half3 rawNormal = normalize(bladeDirection + cameraTransformForwardWS * -0.5 + ...);
half3 N = normalize(lerp(rawNormal, float3(0, 1, 0), _NormalUpwardBias));
`

### B. Gradient Tinting
**File**: Assets/8-13-2026/Grass/Materials/GenshinGrassFoliage.shader

- _RootColor (pangkal/tanah) = (0.12, 0.18, 0.06) — gelap mendekati warna tanah
- _TipColor (ujung/matahari) = (0.65, 0.85, 0.25) — hijau-kuning cerah menangkap sinar
- _GradientPower = 1.5 — kontrol ketajaman gradasi

### C. Fake SSS (Subsurface Scattering / Translucency)
**File**: Diterapkan di kedua shader

Saat pemain melihat ke arah matahari terbenam (backlighting), ujung bilah rumput bersinar keemasan:
`hlsl
half3 backLightDir = normalize(light.direction + N * 0.3);
half backDot = saturate(dot(-V, backLightDir));
half sss = pow(backDot, _SSSPower) * _SSSStrength * positionY/heightFactor;
result += _SSSColor * sss * light.color;
`
- _SSSColor = (0.9, 0.75, 0.2) — glow keemasan
- _SSSStrength = 0.5-0.6
- _SSSPower = 4.0 (ketajaman sudut backlit)

---

## Point 143 – Penegasan Aturan Kerja AI, Protokol Tanpa Inisiatif Liar, dan Pemulihan Stabilitas Shader

### A. Penegasan Aturan Mutlak User
- **Instruksi User**: Penegasan keras bahwa AI dilarang keras mengambil inisiatif sendiri di luar perintah langsung User.
- **Pencatatan Protokol**: Seluruh aturan perilaku, batasan kerja AI, serta visi dan rencana User telah ditambahkan secara resmi di bagian pembuka Dokumentasi.md agar selalu menjadi acuan utama di setiap sesi kerja berikutnya.

### B. Pemulihan Stabilitas Shader (Penyebab & Solusi Layar/Aset Ungu)
- **Masalah**: Aset lingkungan (dinding rumah nenek/gudang) sempat berwarna ungu (*missing shader*).
- **Akar Masalah**: 
  1. Header di StylizedGrass_Mesh.shader memiliki tanda hubung [Header(Stylized Look - Upward Normals)] yang memicu syntax error line 49 pada parser ShaderLab Unity.
  2. Cel.hlsl sempat memanggil fungsi SampleSH() yang tidak tersedia di semua pass render, memicu kegagalan kompilasi pada shader cel anime bersama.
- **Solusi Tuntas**:
  1. Syntax header pada StylizedGrass_Mesh.shader dinormalisasi ke format standar ShaderLab.
  2. Cel.hlsl disterilkan dari dependensi eksternal dan diganti dengan *ambient luminous floor* yang sepenuhnya aman dan mandiri.
  3. CelShaderV2.shader dikembalikan ke status bersih originalnya.
